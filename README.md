...
- 角色卡（按频道隔离，含装备与卡面图）
- 战术地图：多楼层对象模型（矩形/圆形房间 + 无需命名的通道走廊 + 门/梯）。移动可达 = 真实几何相邻（房间/通道贴边或轻切 1 格内即相通；圆形房间用通道伸到圆周旁即可连），锁着的门挡住去路，跨层用“梯”门。玩家按所在楼层看布局，`!go` 沿可达网移动，GM 编辑/显示双模式与一键公开
- 一键导出频道日志（.txt）
...
| `!go <房间名>` | 沿"相邻(贴边/轻切)+未锁门/梯"移动到目标房间（圆形房间用通道轻切进圆周相连；进入未揭示房间会自动揭示） | `!go 医疗舱` |
...
点顶栏「地图」打开。地图是**多楼层房间对象模型**——GM 在每一层画出矩形/圆形房间与通道走廊（相贴/轻切即互通），可加门（锁障）/梯（跨层）；房间可被 GM 隐藏（房间级迷雾）。
...
| 战术地图 | 每频道一张，绘制矩形/圆形房间与通道，用门/梯子连起来，指定入口与已揭示房间（见下节） |
...
每频道一张**多楼层**地图。基础元素：**房间**（矩形/圆形，可命名）、**通道**（细长走廊，连通媒介）、**门 / 梯**（可选锁障 / 跨层通道）。连通规则：**同层任意两对象真实距离 ≤1 格（贴边或轻切）即相通**，无需显式连线；门只作"锁"，梯用于跨层。
...
| 画矩形/圆形房间 | 选「矩形 / 圆形」→ 拖拽 → 松开后弹出**确认+命名**对话框 →「确认创建」 |
| 画通道 | 选「通道」→ 拖拽出细长矩形，**松开即建、无需命名**。伸到任何对象旁即可互通 |
| 拖拽预览 | 拖拽中实时显示尺寸与状态色：**绿=已连通**（贴边/轻切 1 格内）、黄=需再拉近、红=穿入过深无法确认。房间之间只禁过度重叠，通道可轻微切入（≤1 格）以连上圆形/矩形 |
| 连通方式 | **几何相邻即相通**：矩形房间之间可贴边；圆形房间没有直边，用「通道」伸到圆周旁 1 格内（或轻切进圆周 ≤1 格）即可连通，创建时会自动吸附提示 |
